...
// Uses The Graph Protocol queries           </p>
        ...
        <div className="text-center py-8">
          ...
          </p> event state for reliable historical data
      ...
      {isLoading ? (
        ...
        <div className="text-center py-8">
          <div className="w-12 h-12 sm:w-16 sm:h-16 border border-red-500 mx-auto mb-4 flex items-center justify-center">
            <span className="font-gilbert text-xl sm:text-2xl text-red-400">⚠️</span>
          </div>
        ...
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {sortedEvents.slice(0, 10).map((event, index) => (
            <div
              ...
              className="bg-dark border border-army-green-light p-3"
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="text-lg">{getEventIcon(event.type)}</span>
                  <span
                    className={`font-gilbert font-bold ${getEventColor(
                      event.type
                    )}`}
                  >
                    {getEventName(event.type)}
                ...
                <span className="font-gilbert text-army-green-lighter text-xs">
                  {formatTimestamp(Number(event.timestamp))}
                </span>
              </div>
              ...
              <div className="ml-7 space-y-1">
                ...
                {event.penalty && (
                  <p className="font-gilbert text-red-400 text-sm">
                    Penalty: {formatTokenAmount(event.penalty)} STAKE
                  </p>
                )}
                <p className="font-gilbert text-army-green-lighter text-xs">
          ...
          {hasNextPage && (
            <div className="text-center pt-4">
              <button
                onClick={() => fetchNextPage()}
                disabled={isFetchingNextPage}
                className="font-gilbert text-army-green-lighter hover:text-army-green transition-colors disabled:opacity-50"
              >
                {isFetchingNextPage ? "Loading..." : "Load More"}
              </button>
            </div>
